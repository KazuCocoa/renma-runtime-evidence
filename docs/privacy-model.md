...
The 2026-08-07 API-key-backed characterization retained only `exit-nonzero`, false artifact/evidence booleans, `no-otlp-request`, and `inconsistent` for each row. An isolated follow-up reduced its discarded failure stream immediately to the fixed `quota-or-rate-limit` category. Raw stdout and stderr were not retained, every temporary authentication and workspace root was removed, and no user Codex configuration or saved authentication was persisted or modified. This failed process state supports no inference about Skill availability, context injection, selection, reading, or execution.
