...
const syntheticSkill = "renma-activation-explicit-single-20260805";
...
test("closeAndSnapshot drains an accepted in-flight metrics request", async () => {
  const collector = await startLocalCollector("explicit-single");
  ...
  try {
    ...
    assert.equal(presence.verifiedStatus, "ok");
    assert.match(
