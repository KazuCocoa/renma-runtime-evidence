...
All 24 Codex parent processes exited with status `0`. No scenario was unsupported by the installed non-interactive surface.

Phase A showed complete depth-two and depth-three Skill-label presence in all three runs. Branching was not consistently complete: the simple branch observed only the root in one run and root plus branch A in two runs, never branch B; the diamond observed all four labels in two runs and omitted branch B in one run. A missing expected label remains an observation, not a harness failure and not evidence that a file was unread or an instruction was ignored.

In Phase B, the dormant custom-agent configuration produced only the explicitly invoked dormant orchestrator label and no accepted role in all three runs. The child-only Skill label appeared at the same collector as the parent orchestrator in all three single-child runs. The reused depth-two chain's three labels appeared with the parent orchestrator in all three nested-child runs. Both parallel child labels appeared with the parent orchestrator in two runs; neither child label appeared in the remaining run.

No exact allowlisted synthetic role was observed from `codex.multi_agent.spawn` in any Phase B run. The experiment therefore found process-tree-wide co-presence of parent and child-only Skill labels, but it did not find the allowlisted role signal needed even for role/Skill co-presence. It cannot establish which agent injected any Skill. It also cannot distinguish an absent metric export from a metric data point discarded because its `role` was outside the finite allowlist; raw OTLP is intentionally never retained.
...
| `nested-branch`           |   1 | `[renma-topology-branch-root-20260806]`                                                                                                                              | `[]`           |
...
| `nested-diamond`          |   2 | `[renma-topology-diamond-branch-a-20260806, renma-topology-diamond-branch-b-20260806, renma-topology-diamond-root-20260806, renma-topology-diamond-shared-20260806]` | `[]`           |
| `nested-diamond`          |   3 | `[renma-topology-diamond-branch-a-20260806, renma-topology-diamond-root-20260806, renma-topology-diamond-shared-20260806]`                                           | `[]`           |
...
| `subagent-parallel`       |   1 | `[renma-topology-orchestrator-parallel-20260806]`                                                                                                                    | `[]`           |
...
| `nested-branch`        | root → branch A and branch B        | Root observed in 3/3, branch A in 2/3, branch B in 0/3.             |
| `nested-diamond`       | root → branches A/B → shared target | Root, branch A, and shared target observed in 3/3; branch B in 2/3. |
...
| `subagent-parallel`       | orchestrator → alpha/beta roles → distinct child Skills            | Parent plus both child labels in 2/3; parent only in 1/3; no accepted role. |
...
The child-only Skill observations show that those labels reached the same loopback collector as the parent process's labels under this configuration. They do not prove that a particular role used a particular Skill. Because no allowlisted spawn role was observed, role-level correlation is additionally inconclusive in this sample.
...
- launched one separate parent `codex exec --ephemeral` process with `--ignore-user-config`, `--ignore-rules`, and explicit multi-agent enablement;
...
npm ci
npm run check
npm run experiment:codex:skill-topology-boundaries -- --runs 3
git diff --check
```
